...
for i in range(Totalx):
    ...
    ImPsi[i] = sin(k0*i*delx) * exp(-(i-start)**2/sigma**2) #if it's going to be derived from E (I think?)
    #I rememeber checking how delx needed to be incorporated into the trig functions:
    #I believe I remember finding that this inclusion of delx created a consistent period in the RealPsi graph for a 
    #given wavelength and sigma, if sigma is to also be measured in units of delx
...
def animate(j):
    ...
    else: #Here's the integrator, it runs when the program is paused (k and integrate are used to make sure we dont
        #integrate over and over again while the animation is paused)
        if k < integrate:
            transmitted = 0
            reflected = 0
            total = 0
...
print("Above is the animation. Click on the graph to pause it. \n")
print("Once paused, Transmission and Reflection coefficients will be displayed. \n")
print("These coefficicents will be recalculated each time the animation is paused. \n")
